...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1, num_extra_inputs=0, embedding_shift_enabled=False):
        super(ResNet, self).__init__()
        self.in_channels = 64
        self.embedding_shift_enabled = embedding_shift_enabled
        ...
        self.fc = nn.Linear(512 * block.expansion + num_extra_inputs, num_classes)

        if self.embedding_shift_enabled:
            self.embedding_shift = nn.Parameter(torch.randn(512 * block.expansion), requires_grad=False)
...
# +
# resnet_for_regression = ResNet(
#     block=BasicBlock, 
#     layers=[2, 2, 2, 2],  # This is the configuration for ResNet-18
#     num_classes=1,  # For a single regression target
#     num_extra_inputs=0,  # Adjust if you have extra inputs
#     embedding_shift_enabled=False  # Enable or disable embedding shift
# )

# +
# regression_model = RegressionModel(
#     model=resnet_for_regression,
